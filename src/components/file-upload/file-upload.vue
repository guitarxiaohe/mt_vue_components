...
const displayErrorMessage = computed(
  () => currentUpload.value.displayErrorMessage.value
);
  ...
  <div class="file-upload">
    ...
    <div v-if="isMultiple && !isEmpty" class="file-list">
      ...
      >
        <!-- 上传中状态 -->
        <FileStatus
          v-if="multipleFileUpload.getUploadItem(file)?.status === 'uploading'"
          status="loading"
          :file-name="file.name"
        ...
        <!-- 错误状态 -->
        <FileStatus
          v-else-if="multipleFileUpload.getUploadItem(file)?.status === 'error'"
          status="error"
          :file="file"
          :file-name="file.name"
          :icon-class="multipleFileUpload.getFileIconClass(file)"
